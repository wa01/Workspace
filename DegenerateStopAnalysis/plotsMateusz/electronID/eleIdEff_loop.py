...
preSel3 = "(Max$(Jet_pt*(abs(Jet_eta) < 2.5) > 100))" #ISR
...
for i in range(nEvents):
   ...
   for recoEle in recoEles:
      if abs(recoEle['pdgId']) == 11 and abs(recoEle['eta']) < 2.5 and recoEle['mcMatchId'] != 0 and recoEle['matchIndex'] != -1 and recoEle['deltaRmin'] < deltaRcut: #or recoEle['matchIndexTau'] != -1
         ...
         for i in range(1,5):
            if recoEle['SPRING15_25ns_v1'] >= i: 
               ...
               elif zoom == True: hists_passed[i-1].Fill(genEles[recoEle['matchIndex']]['pt'], weight/2) 
         ...
         if recoEle['mvaIdSpring15'] >= MVA_min1: 
            ...
            elif zoom == True: hists_passed[4].Fill(genEles[recoEle['matchIndex']]['pt'], weight/2) 
         ...
         if recoEle['mvaIdSpring15'] >= MVA_min2:
            ...
            elif zoom == True: hists_passed[5].Fill(genEles[recoEle['matchIndex']]['pt'], weight/2) 
   ...
   for genEle in genEles:
      ...
      if abs(genEle['pdgId']) == 11 and abs(genEle['eta']) < 2.5 and len(genEles) == nEles:
         ...
         elif zoom == True: hist_total.Fill(genEle['pt'], weight/2) 
...
if save == True:
   ...
   #Save to Web
   c1.SaveAs(savedir + "eleIDeff_loop_" + inputSample + z + ".root")
   c1.SaveAs(savedir + "eleIDeff_loop_" + inputSample + z + ".png")
   c1.SaveAs(savedir + "eleIDeff_loop_" + inputSample + z + ".pdf")
